...
parser.add_argument("-a","-aout",default=0,type=float,help="Outer Exponent [0]")
...
parser.add_argument("-Mgh",default=0.55,type=float,help="Absolute magnitude of star in  the halo [-0.55]")

parser.add_argument("-Mgd",default='0.55',type=str,help="Absolute magnitude of star in the disc [-0.55]")


parser.add_argument("-Mgc",default=0.,type=float,help="Gaussian centroid")
parser.add_argument("-Mgs",default=1,type=float,help="Gaussian dispersion")

parser.add_argument("-agecut",default=[-999,-999],nargs='+',type=float,help="Age cut of the isochrones (None,None)")
parser.add_argument("-fecut",default=[-999,-999],nargs='+',type=float,help="Metallicity cut of the isochrones (None,None)")
parser.add_argument("-colcut",default=[-1.0,-0.4],nargs='+',type=float,help="j-g color cut of the isochrones (-1.0,-0.4)")
parser.add_argument("-Mgcut",default=[-3,6],nargs='+',type=float,help="Mg cut of the isochrones (-3,6)")
...
if args.wd!=1:
	...
	oldgh=ut.dist_to_g(disth,Mg=args.Mgh)
	print('Done')
	print('Masking....',end='')
	lh,bh,gh,idxh=mod.halo_mask(lh,bh,oldgh,bmin=bmin,gmagmin=gmin,gmagmax=gmax)
	disth=disth[idxh]
	print('Done')
	npartsh=len(lh)
...
if args.wd!=0:
	...
	if isinstance(Mgd,float):
		...
		oldgd,agd=mod.gau_mag(distd,Mg0=args.Mgc,Mgs=args.Mgs)
	else:
		oldgd,agd,agd_input=mod.disc_mag(Mgd,distd,agecut=agecut,fecut=fecut,gcut=Mgcut,colcut=colcut)
	print('Masking....',end='')
	ld,bd,gd,idxd=mod.halo_mask(ld,bd,oldgd,bmin=bmin,gmagmin=gmin,gmagmax=gmax)
	distd=distd[idxd]
	print('Done')
	npartsd=len(ld)
...
if args.wd!=0:
	...
	if args.Mgd[0].lower()=='f':
		...
		dict_masked['Mgc']=args.Mgc
		dict_masked['Mgs']=args.Mgs
		dict_global['Mgc']=args.Mgc
		dict_global['Mgs']=args.Mgs
	else: pass
...
if args.wd==0:
	...
	ra=cir.ra.degree
	dec=cir.dec.degree
	ut.make_fits({'X':x,'Y':y,'Z':z,'m':m,'r':r,'rcyl':rcyl,'idh':np.ones_like(r),'idd':np.zeros_like(r),'Mg':[args.Mgh,]*len(r)},outname=outdir+'/'+name+'_global.fits',header_key=dict_global)
	ut.make_fits({'X':x[idxh],'Y':y[idxh],'Z':z[idxh],'l':lh,'b':bh,'distance':disth,'g':gh,'Mg':[args.Mgh,]*len(lh),'ra':cir.ra.degree,'dec':cir.dec.degree,'idh':np.ones_like(lh),'idd':np.zeros_like(lh)},outname=outdir+'/'+name+'_masked.fits',header_key=dict_masked)
